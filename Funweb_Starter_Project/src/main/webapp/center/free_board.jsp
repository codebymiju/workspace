	...
	<div id="wrap">
		...
		<article>
			...
			<table id="notice">
				...
				<tr onclick="location.href='free_board_content.jsp?idx=<%=freeBoard.getIdx()%>&pageNum=<%=pageNum%>'">
			...
			<div id="table_search">
				<input type="button" value="글쓰기" class="btn" onclick="location.href='free_board_write.jsp'">
			</div>
			<!-- 검색 기능(driver.jsp 페이지 하나로 일반 목록과 검색어 목록 통합) -->
			<div id="table_search">
			...
			<div id="page_control">
				...
				// 1. BoardDAO 객체의 selectListCount() 메서드를 호출하여 전체 게시물 수 조회
				//		 (검색 기능 통합으로 인해 키워드도 함께 전달)
				// (페이지 목록 계산에 사용)
				int listCount = dao.selectFreeBoardListCount(keyword);
